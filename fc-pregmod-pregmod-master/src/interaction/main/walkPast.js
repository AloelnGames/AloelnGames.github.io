...
window.walkPast = (function() {
	...
	function walkPast(activeSlave, fixed) {
		...
		if (V.familyTesting === 1 && totalRelatives(activeSlave) > 0 && ((fixed && V.target === "FRelation") || (!fixed && jsRandom(1, 100) > 80))) {
			...
			} else if (activeSlave.mother === V.relation.ID) {
				V.relationType = "mother";
			} else if (activeSlave.father === V.relation.ID) {
				V.relationType = "father";
			} else {
				switch (areSisters(activeSlave, V.relation)) {
					case 1:
						V.relationType = "twin";
						break;
					case 2:
						V.relationType = "sister";
						break;
					case 3:
						V.relationType = "half-sister";
						break;
				}
			}
			flag = 110;
		...
		} else if (activeSlave.rivalry !== 0 && hasAllLimbs(activeSlave) && ((fixed && V.target === "FRelation") || (!fixed && jsRandom(1, 100) > 70))) {
			flag = 130;
			V.partner = "rivalry";
		} else {
			flag = 99;
		...
		if (fixed && (["FRelation", "FRival"].includes(V.target) && !(V.partner))) {
			return;
		}
		...
		if (V.partner === "rivalry") {
			...
			if (activeSlave.belly < 150000) {
				if (activeSlave.dick > 0 && (V.sexualOpeness === 1 || activeSlave.toyHole === "dick")) {
			...
			if (fixed) {
				...
				watch = jsEither(watchArray);

				if (watch === "boobWatch") {
					output += boobWatch(activeSlave);
				} else if (watch === "dickWatch") {
					output += dickWatch(activeSlave);
				} else if (watch === "vaginaWatch") {
					output += vaginaWatch(activeSlave);
				} else if (watch === "bellyWatch") {
					output += bellyWatch(activeSlave);
				} else if (watch === "anusWatch") {
					output += anusWatch(activeSlave);
				} else if (watch === "buttWatch") {
					output += buttWatch(activeSlave);
				} else if (watch === "lipWatch") {
					output += lipWatch(activeSlave);
				}
			}
		}
		...
		if (activeSlave.fuckdoll === 0) {
			if (watch === "buttWatch" && activeSlave.chastityAnus) {
				output += ` If you wish to have anal intercourse with ${him} you must order ${him} to remove ${his} chastity belt.`;
				output += ` <span id="walkpast"><<link "Grope ${his} rear instead">><<replace "#walk">><<include $target>><</replace>><</link>></span>`;
			} else if (watch === "vaginaWatch" && activeSlave.chastityVagina) {
				output += ` If you wish to have vanilla intercourse with ${him} you must order ${him} to remove ${his} chastity belt.`;
			} else if (flag > 100) {
				if (V.familyTesting === 1 && flag === 110) {
					/* No content yet */
				} else {
					output += ` <span id="walkpast"><<link "Summon them both">><<replace "#walk">><<include $target>><</replace>><</link>></span>`;
				}
				...
				if (["be confined in the arcade", "be confined in the cellblock", "be the Attendant", "be the DJ", "be the Madam", "be the Milkmaid", "be the Schoolteacher", "be the Stewardess", "be the Wardeness", "be your agent", "be your Concubine", "live with your agent", "live with your Head Girl", "serve in the club", "serve in the master suite", "stay confined", "work a glory hole", "work as a servant", "work in the brothel", "work in the dairy"].includes(activeSlave.assignment)) {
	...
	function loverSlave(activeSlave) {
		...
		if (partnerSlave !== undefined) {
			...
			const {
				...
				/* /* eslint-enable no-unused-vars */
			partnerName = partnerSlave.slaveName;
			race2 = (V.seeRace ? partnerSlave.race : "");
					...
					switch (activeSlave.fetish) {
						...
						case "dom":
							...
							if (canPenetrate(activeSlave) && fuckSeed > 50) {
								...
								if (canPenetrate(partnerSlave) && activeSlave.vagina !== 0) {
									if (fuckSeed > 45 && hasBothArms(_partnerSlave)) {
					...
					switch (slave.clothes) {
						...
						case "overalls":
							if (slave.boobs > (slave.belly+250)) {
								t += `Yourslave.slaveName's giant breasts push out ${his} overalls so far that ${his} hugely fat belly is left uncovered.`;
							} else {
				...
				if (slave.bellyImplant >= 32000) {
					...
					switch (slave.clothes) {
						...
						case "a mini dress":
							...
							break;
						case "a monokini":
							t += `${His} monokini struggles to reign in ${his} giant gut.`;
							break;
						case "overalls":
							if (slave.boobs > (slave.belly+250)) {
